	...
	SubShader
	{
		...
		Pass
		{
			...
			float snoise( float3 v )
			{
				...
				float3 IN = floor( v + dot( v, C.yyy ) );
				float3 x0 = v - IN + dot( IN, C.xxx );
				...
				float3 x3 = x0 - 0.5;
				IN = mod3D289( IN);
				float4 p = permute( permute( permute( IN.z + float4( 0.0, i1.z, i2.z, 1.0 ) ) + IN.y + float4( 0.0, i1.y, i2.y, 1.0 ) ) + IN.x + float4( 0.0, i1.x, i2.x, 1.0 ) );
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float clampResult47 = clamp( pow(abs (dotResult42*0.5 + 0.5) , _SunFogRange ) , 0.0 , 1.0 );
