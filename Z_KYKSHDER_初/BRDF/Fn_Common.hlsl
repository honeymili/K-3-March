...
void GetCurvature_float(float SSSRange,float SSSPower,float3 WorldNormal,float3 WorldPos,out float Curvature)
{
	Curvature = 1.0;
	#ifndef SHADERGRAPH_PREVIEW
	float deltaWorldNormal = length(abs(ddx(WorldNormal))) + length(abs(ddy(WorldNormal)));
	float deltaWorldPosition = length(abs(ddx(WorldPos))) + length(abs(ddy(WorldPos))) / 0.001;
